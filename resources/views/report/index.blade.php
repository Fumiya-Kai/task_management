  ...
  <table class="targets-table">
    <thead class="thead-index">
      <tr class="row-index tr-index">
        <th class="col col1">日付</th>
        <th class="col col2">タイトル</th>
      </tr>
    </thead>
    <tbody>
      @foreach ($reports as $report)
      <tr class="row-index tr-index target-data" data-report-id="{{ $report->id }}">
        <td class="col col1" width="5%">{{ $report->report_time->format('m/d(D)')}}</td>
        <td class="col col3" width="45%">{{ $report->title }}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
